...
(PROTOTYPE) Provides the means to uniquely identify/validate the integrity of any EPCIS event through a common, syntax-agnostic approach based on hashing. Takes an EPCIS event (formatted in either XML or JSON-LD) and returns a hash value representing a unique fingerprint of the latter.
