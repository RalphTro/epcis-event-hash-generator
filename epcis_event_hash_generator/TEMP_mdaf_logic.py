...
def get_term_mapping(term: str, context_map: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
    ...
    if isinstance(entry, dict):
        ...
        # Some contexts use string aliasing even inside dict—fallback
        # (unlikely for your GS1 context, but kept for robustness)
        for k in ("@id", "id"):
...
def resolve_property_identifier(prop: str, context_map: Dict[str, Any], prefixes: Dict[str, str]) -> str:
    ...
    Resolve a property name to a printable identifier according to rules:
      ...
      - Bare name:
          ...
          - If '@id' is non-GS1 ('schema:*', etc.) -> bare non-GS1 not allowed -> error
    """
    ...
    if is_curie(prop):
        # Expand GS1 CURIE to full IRI; keep other vocabularies as CURIEs (allowed)
        if prop.startswith("gs1:"):
            return resolve_curie(prop, prefixes)
        return prop  # non-GS1 CURIE stays as-is

    # Bare term: must be in context
    mapped, _entry = get_term_mapping(prop, context_map)
    if mapped.startswith("gs1:"):
        return resolve_curie(mapped, prefixes)
    # Bare but mapped to non-GS1 -> not allowed
    raise ValueError(
...
def is_code_list_property(prop: str, context_map: Dict[str, Any]) -> bool:
    """
    Determine if a property is a code-list property by checking if its context entry has an '@context' enumeration.
    Supports both 'prop' and 'gs1:prop' keys in the top-level context.
    """
    entry = context_map.get(prop)
    if not isinstance(entry, dict) or "@context" not in entry:
        # try gs1:prop variant (some contexts define both)
...
def get_code_enumeration(prop: str, context_map: Dict[str, Any]) -> Dict[str, str]:
    """
    Return { code_key -> mapped_identifier } from the property's '@context' enumeration.
    mapped_identifier is typically a CURIE (e.g., 'gs1:LocationID_Type-UN_LOCODE').
    """
    entry = context_map.get(prop)
    if not isinstance(entry, dict) or "@context" not in entry:
        entry = context_map.get(f"gs1:{prop}")
    enum_map = entry.get("@context", {}) if isinstance(entry, dict) else {}
    # Only keep str identifiers
...
def normalize_code_value(prop: str, value: Any, context_map: Dict[str, Any], prefixes: Dict[str, str]) -> Any:
    ...
    if not isinstance(value, str):
        return value  # only strings are normalized as codes

    enum_map = get_code_enumeration(prop, context_map)
    if not enum_map:
        return value  # not a code-list

    # Build full-IRI lookup for all allowed codes
    # enum_map: code_key -> (usually) 'gs1:ClassPrefix-CODE'
    allowed_full_iris = set()
    code_to_full_iri = {}

    for code_key, mapped_identifier in enum_map.items():
        full_iri = resolve_curie(mapped_identifier, prefixes)
    ...
    # Bare strings:
    # 1) If it's exactly a code key (e.g., 'UN_LOCODE')
    if value in code_to_full_iri:
        return code_to_full_iri[value]

    # 2) If it's a local name like 'LocationID_Type-UN_LOCODE'
    #    Try to interpret by prepending 'gs1:' and expanding; then check set.
...
def process_object(obj: Dict[str, Any], context_map: Dict[str, Any], prefixes: Dict[str, str]):
    ...
    Traverse a master-data object:
      - print id/@id and type/@type as 'id=' / 'type=' (no URI expansion)
      - resolve GS1 properties to full IRIs dynamically via context/prefixes
      - non-GS1 bare properties -> error; non-GS1 CURIE/IRI -> allowed (printed as given)
      - normalize code-list values to full GS1 IRI
      - recurse into nested dicts and lists
    """
    for prop, value in obj.items():
        # Normalize 'id' / '@id' and 'type' / '@type'
        if prop in ("id", "@id"):
            print_kv("id", value)
            continue
        if prop in ("type", "@type"):
            print_kv("type", value)
            continue

        # Containers: validate property, then recurse
        if isinstance(value, dict):
            # Validate property name (may raise)
            _ = resolve_property_identifier(prop, context_map, prefixes)
        ...
        if isinstance(value, list):
            ...
            # If list contains objects, recurse; else print values
        ...
        # If it's a code-list property, normalize the value to full GS1 IRI
        if is_code_list_property(prop, context_map):
...
def main():
    ...
    # Top-level @context must be an object
    ...
    # Only execute if gs1:masterDataAvailableFor appears in any event
    if not any("gs1:masterDataAvailableFor" in event for event in events):
        print("No gs1:masterDataAvailableFor found. Exiting.")
        return

    print("gs1:masterDataAvailableFor detected")
